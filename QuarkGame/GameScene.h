...
#include "../QuarkEngine/Header/InputManager.h"
#include <PathUtils.h>
#include "Scene.h"
...
public:
    ...
    void OnUpdate(float dt) override
    {
        ...
        if (engine->getInputManager().IsKeyDown(SDL_SCANCODE_LEFT) || engine->getInputManager().IsKeyDown(SDL_SCANCODE_A))
            paddleTrans->x -= paddleSpeed * dt;
        if (engine->getInputManager().IsKeyDown(SDL_SCANCODE_RIGHT) || engine->getInputManager().IsKeyDown(SDL_SCANCODE_D))
            paddleTrans->x += paddleSpeed * dt;
